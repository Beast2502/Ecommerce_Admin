...
const AddDetails = (props) => {
  ...
  const handleSubmit = (event) =>{
      ...
      props.addProduct({addName ,addCat , addDesc , addQuan ,addImage , addCol ,addSku ,addWeig ,addLen ,addWid ,addHei})
  ...
  return (
    <div className="product-container">
      <p>Add product</p>
      <div className="">
        <InputField name={"Product Name"} type={'text'} handleChange={handleName}/>
        <InputWithSearch name={"Catogory Name"} list={categoryList} onChange={handleCategory} />
        <InputField name={"Product Description"} handleChange={handleDesc}/>
        <InputField name={"Quanity"} type={'number'} handleChange={handleQuant} />
        ...
        <InputField name={"Item Weight"} type={'number'} handleChange={handleWeight} />
        <div>
          <p>Item Dimension</p>
          <InputField name={"Lenghth"} type={'number'} handleChange={handleLen} />
          <InputField name={"Width"} type={'number'} handleChange={handleWid} />
          <InputField name={"Height"} type={'number'} handleChange={handleHei} />
        </div>

        <Button name={'Add Product'} color ={'rgb(223, 66, 66);'} onClick={handleSubmit}/>
      </div>
    </div>
  );
};
